...
def set : category Σ α : Type u, fintype α :=
{ Mor := λ S T, S.1 → T.1,
  Comp := λ S T U, (∘),
  Id := λ S, @id S.1,
  Hid_left := λ S T f, rfl,
...
def Grp : category Σ α : Type u, group α :=
...
def Top : category Σ α : Type u, topological_space α :=
...
def Ab : category Σ α : Type u, add_comm_group α :=
...
def Mod (α : Type u) [ring α] : category Σ β : Type v, module α β :=
...
def of_poset (α : Type u) [partial_order α] : category α :=
{ Mor := λ x y, plift $ x ≤ y,
...
def Preord : category Σ α : Type u, preorder α :=
