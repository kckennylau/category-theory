...
def functor_category {α β} (C : category.{u v} α) (D : category.{u₁ v₁} β) : category (functor C D) :=
...
def cone_category {α β} (J : category.{u v} α) (C : category.{u₁ v₁} β) (F : functor J C) :
...
def binary_product {α} (C : category.{u v} α) (x y : α) :=
limit (discrete bool) C (functor.of_discrete C $ λ b, bool.rec_on b x y)

def product {α} (C : category.{u v} α) (ι) (f : ι → α) :=
limit (discrete ι) C (functor.of_discrete C f)

def product_category {α β} (C : category.{u v} α) (D : category.{u₁ v₁} β) : category (α × β) :=
...
def Hom {α} (C : category.{u v} α) : functor (product_category (dual C) C) examples.Set :=
{ F := λ x, C.Mor x.1 x.2,
